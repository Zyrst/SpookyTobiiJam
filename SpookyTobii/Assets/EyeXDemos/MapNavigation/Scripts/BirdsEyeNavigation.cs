    ...
    public void Update()
    {
        ...
        else
        {
            ...
            if (!Input.GetKey(KeyCode.Space))
            {
                ...
                var step = _pannableComponent.Step;

                /*
                  Here we check if we got panning or stepping.
                  Panning is when the user press and hold the EyeX button while
                  looking around at a pannable area, while stepping is
                  "teleportation" within a pannable area.
                */

                // Got panning?
                if (velocity != Vector2.zero && step == Vector2.zero)
                {
                ...
                // Got stepping?
                else if (step != Vector2.zero)
                {
                    Debug.Log("Velocity: " + velocity);

                    // Calculate the new position to go to.
                    var newPosition = Camera.main.WorldToScreenPoint(_sourcePosition) + new Vector3(-step.x, step.y, 0);
                    SetTargetPosition(Camera.main.ScreenToWorldPoint(newPosition));
                    SetTargetSize(_initialSize);
                }
            }   
        }
